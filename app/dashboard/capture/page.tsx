...
export default function CapturePage() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      // Fetch forges that are available for capture
      const allForges = await dataStore.getForges()
      const captureForges = allForges.filter((f) => f.state === "CREATED" || f.state === "CAPTURED")
      setForges(captureForges)

      // Fetch all models and create a map
      const allModels = await dataStore.getModels()
      const modelMap = new Map<string, Model>()
      allModels.forEach((m) => modelMap.set(m.id, m))
      setModels(modelMap)

      // Check URL for pre-selected forge
      const forgeIdFromUrl = searchParams.get("forge")
      if (forgeIdFromUrl && captureForges.some((f) => f.id === forgeIdFromUrl)) {
        setSelectedForgeId(forgeIdFromUrl)
      }
    } catch (error) {
      console.error("[v0] Error loading capture data:", error)
    } finally {
      setIsLoading(false)
    }
  }
